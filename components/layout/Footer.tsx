      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-4 gap-12">
          <div className="md:col-span-2">
            ...
            <p className="text-brand-muted max-w-md mb-6">
              Transformamos operações através de automação inteligente
            ...
            <div className="flex items-center gap-4">
              {[
                { icon: Linkedin, href: '#' },
                { icon: Instagram, href: '#' },
                { icon: Mail, href: 'mailto:contato@alma.ai' },
              ].map((social, i) => (
                <motion.a
                  key={i}
                  href={social.href}
                  whileHover={{ scale: 1.1, y: -2 }}
                  className="w-10 h-10 rounded-full bg-brand-surface flex items-center justify-center text-brand-muted hover:text-brand-primary hover:bg-brand-primary/10 transition-colors"
                >
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-sm text-brand-muted">
            {new Date().getFullYear()} ALMA. Todos os direitos reservados.
          </p>
